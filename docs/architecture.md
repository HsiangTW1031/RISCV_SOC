...
flowchart TB
    ...
    XBAR["axi_lite_xbar<br/>手刻 AXI4-Lite crossbar<br/>2 master (s0=CPU 優先, s1=JTAG) x 9 slave"]
...
- **9 個 slave**：ROM、RAM、Timer、Watchdog、UART、I2C、SPI、AES、DMA（控制埠)。位址解碼在 `decode_addr()`,未命中的位址由 crossbar 自己回 SLVERR,不會誤觸真正的 slave。
...
已知限制（Phase 1 就存在,一直沒改)：`picorv32_axi` 這個 adapter 完全沒接 BRESP/RRESP pin,不會檢查 SLVERR。踩到未映射位址不會讓匯流排卡死,但 CPU 自己也不會發現。
