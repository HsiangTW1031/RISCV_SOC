...
For the algorithm background (why AES/Rijndael, its FIPS-197 standardization, and why it's considered secure), see `docs/aes_report.md` — this document only covers the RTL interface.
...
合成/timing：`blocks/aes/syn/`、`blocks/aes/sta/` 有針對 `aes_core`（不含 AXI wrapper）在 Nangate45 下的獨立合成 + STA 結果,詳見 `docs/aes_report.md`。
