...
加入 `docs/verification/cdc_methodology.md` 的 reset synchronizer 之後,`equiv_simple` 一開始直接報錯:「No SAT model available for async FF cell ... Consider running `async2sync`」——這個設計唯一真正非同步 reset 的正反器就是新加的 reset synchronizer 本身,SAT 沒有現成模型處理。在 gold/gate 兩邊的 `prep -flatten` 之後都加上 Yosys 內建的 `async2sync` pass(把非同步 FF 輸入轉換成行為等價的同步電路)解決,加完之後數字幾乎不變,確認這個 reset 相關的新增邏輯本身沒有引入新的等價性問題。之後 `blocks/soc_top/syn/synth.ys` 加上 gate-sizing(`docs/verification/performance.md` §1)重跑一次,證明的 checkpoint 數量原封不動,同樣的道理:sizing 不改邏輯,不該也沒有影響證明覆蓋率。
