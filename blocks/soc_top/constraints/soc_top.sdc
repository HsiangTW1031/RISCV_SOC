...
# `resetn` itself is the async input to the two reset synchronizers in
# soc_top.v (resetn_clk_meta/resetn_clk_sync, resetn_tck_meta/resetn_tck_sync)
# -- by design, both flops in each synchronizer have their asynchronous
# clear driven directly by this raw, genuinely-async port (see soc_top.v's
...
# recovery/removal check against `resetn` is inherent to any reset
# synchronizer's first stage, not a real timing bug -- there is no way to
# guarantee a genuinely-asynchronous signal never transitions within a
# clock period's recovery/removal window, and that's precisely the
# metastability risk the synchronizer's second stage exists to absorb.
# `resetn` fans out to nothing else in this design (verified: its only
# remaining reader anywhere is these two always blocks), so exempting
# every path starting at this port is precise, not a broad workaround.
